 ...
 * 
 * code added by: gianna sfrisi (gsfrisi@bu.edu) and ellis hamilton (ellisgham@bu.edu)
 */
...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        /* Replace this line with your implementation of the method. */
            ...
            if (charNum == 0){
            String wordBefore = words[wordNum + 1];
            String testOne = wordBefore.substring(wordBefore.length());
            if (testOne.equals(letter) == false){
                return false;
            }
            }

            String notAddedYet = words[wordNum] + letter;
            if (alreadyUsed(notAddedYet) == true){
                return false;
            }

            addLetter(letter, wordNum);
            String test = words[wordNum];

            if (dictionary.hasString(test) == false){
                return false;
            }

            if(onSameSide(letter, test.substring(charNum -1, charNum)) == true){
                return false;
            }
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        ...
        if (allLettersUsed() == true && dictionary.hasFullWord(words[wordNum]) && words[wordNum].length() >= 3){
